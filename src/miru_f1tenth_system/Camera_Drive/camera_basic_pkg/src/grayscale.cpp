...
{
  public:
    LaneFollowerCameraNode() : Node("lane_follower_camera_node"), previous_error_(0.0), integral_(0.0)
    {
  ...
  private:
    ...
    bool ransacLine(const std::vector<cv::Point> &points, cv::Vec4f &best_line, std::vector<cv::Point> &best_inliers,
                    int iterations = 100, double threshold = 20.0, int minInliers = 500)
    {
        size_t best_count = 0;
        if (points.size() < 2)
            return false;
