...

{
  ...
  public:
...

    {
        ...
        // Camera parameter
        camera_index = this->declare_parameter("camera_index", 0);
        Kp = this->declare_parameter("Kp", 0.005);
        Ki = this->declare_parameter("Ki", 0.0);
        Kd = this->declare_parameter("Kd", 0.0);

        gaus_blur_size = this->declare_parameter("gaus_blur_size", 5);
        thresh = this->declare_parameter("thresh", 100);
        blockSize = this->declare_parameter("blockSize", 11);
        C = this->declare_parameter("C", 10);

        canny_inf = this->declare_parameter("canny_inf", 50);
        canny_sup = this->declare_parameter("canny_sup", 150);

        hough_threshold = this->declare_parameter("hough_threshold", 50);
        hough_inf_pixel = this->declare_parameter("hough_inf_pixel", 50);
        hough_pixel_gap = this->declare_parameter("hough_pixel_gap", 10);
        slope_threshold = this->declare_parameter("slope_threshold", 0.3);

        camera_frame_width = this->declare_parameter("camera_frame_width", 640);
        camera_frame_height = this->declare_parameter("camera_frame_height", 360);
        camera_fps = this->declare_parameter("camera_fps", 30);
        camera_exposure = this->declare_parameter("camera_exposure", 1);
        camera_brightness = this->declare_parameter("camera_brightness", 128);

        speed_sigmoid_k_ = this->declare_parameter("speed_sigmoid_k", 1.0);
        speed_sigmoid_x0_ = this->declare_parameter("speed_sigmoid_x0", 3.0);
        speed_base_ = this->declare_parameter("speed_base", 1.5);
        speed_reduction_ = this->declare_parameter("speed_reduction", 0.9);
        ...
        std::cout << "Camera settings: " << cap_.get(cv::CAP_PROP_FRAME_WIDTH)
            ...
            << " at " << cap_.get(cv::CAP_PROP_FPS) << " fps." << std::endl;
        ...
        // timer_ = this->create_wall_timer(std::chrono::milliseconds(15), std::bind(&LaneFollowingNode::timer_callback, this));
  ...
  private:
    ...
    int camera_index;
    int camera_frame_width, camera_frame_height, camera_fps, camera_exposure, camera_brightness;
    double speed_sigmoid_k_, speed_sigmoid_x0_, speed_base_, speed_reduction_;

    double Kp, Ki, Kd;
    double previous_error_;
    double integral_;

    int thresh, blockSize, C;
    int gaus_blur_size;
